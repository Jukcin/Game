// dragdrop.js - Contains only drag and drop functionality
  ...
  itemElement.addEventListener("dragstart", (e) => {
    ...
    if (
      ...
      if (currentSlotName === "Main Hand") {
        window.player.unequipItem("mainHand");
      } else if (currentSlotName === "Off Hand") {
        window.player.unequipItem("offHand");
      } else if (currentSlotName === "Legs") {
        window.player.unequipItem("legs");
      }
      ...
      window.itemOverlay.appendChild(itemElement);
...
function getAllOtherItems(excludeItem) {
  const items = [];
  const itemElements = window.itemOverlay.querySelectorAll(".inventory-item");
...
function setupDragAndDrop(inventoryGrid, equipmentSlots) {
  ...
  document.addEventListener("drop", (e) => {
    ...
    if (targetSlot) {
      ...
      if (targetSlot.querySelector(".inventory-item")) {
        ...
        window.itemOverlay.appendChild(existingItem);
      }
      ...
      if (type === "sword") {
        if (slotName === "Main Hand") {
          window.player.equipItem("mainHand", draggedItem._item);
        } else if (slotName === "Off Hand") {
          window.player.equipItem("offHand", draggedItem._item);
        }
      } else if (type === "legs" && slotName === "Legs") {
        window.player.equipItem("legs", draggedItem._item);
      }
    ...
    // If dropping on inventory
      ...
      if (
        ...
        window.itemOverlay.appendChild(draggedItem);
      }
...
function placeItemInInventory(item, startRow = 0, startCol = 0) {
    ...
    // Update player's inventory tracking
    window.player.removeInventoryItem(item.name); // Remove if already exists
    window.player.addInventoryItem(item, { row: startRow, col: startCol });
    
    return itemElement;
  }
  
  // In the document drop event handler inside setupDragAndDrop
  // After handling drops in the inventory
  // Add these lines where you update the item position:
  
  if (targetSlot) {
    // ... existing code for equipping items ...
    
    // Update player's inventory tracking - remove it from inventory when equipped
    window.player.removeInventoryItem(draggedItem._item.name);
  }
  else {
    // ... existing code for inventory placement ...
    
    // After successfully placing in inventory, update position tracking
    const col = Math.floor(parseInt(draggedItem.style.left) / cellSize);
    const row = Math.floor(parseInt(draggedItem.style.top) / cellSize);
    
    // Update the inventory tracking with new position
    window.player.removeInventoryItem(draggedItem._item.name); // Remove old entry
    window.player.addInventoryItem(draggedItem._item, { row, col }); // Add with new position
  }
